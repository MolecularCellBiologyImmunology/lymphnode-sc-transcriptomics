...
umimultiplicity <- as.integer(params[['umimultiplicity']])
multiplicitycorrection <- as.logical(params[['multiplicitycorrection']])
...
# umimultiplicity <- as.integer('256')
# multiplicitycorrection <- as.logical('TRUE')
...
# Apply correction for possibility that the same UMI is attached to different
# RNA's of the same gene. See eq. 3 in Grun et al (2014), Nature 
# Methods 11:637--640 http://dx.doi.org/10.1038/nmeth.2930
if (multiplicitycorrection) {
    if (umimultiplicity < 4) {
        # TODO: error message, multiplicity too low
    } else {
        counts_per_cell_and_gene <- counts_per_cell_and_gene %>%
            mutate(cor.reads = -umimultiplicity * log(1 - (reads/umimultiplicity)))
    }
}
