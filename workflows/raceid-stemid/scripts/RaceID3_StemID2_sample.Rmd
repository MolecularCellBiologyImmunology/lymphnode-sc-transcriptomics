...
## Remove Specific Cells and Genes from the Analysis. Objects CellsToRemove and GenesToRemove have to be made during past-analysis, else this step will be skipped.
if (exists("CellsToRemove")) {
  prdata <- prdata[,!(colnames(prdata) %in% CellsToRemove)]
}
if (exists("GenesToRemove")) {
  prdata <- prdata[!(rownames(prdata) %in% GenesToRemove),]
}
...
write.table(x[order(x$cluster,decreasing=FALSE),],file.path(params$outputdirectory,"cell_clust.xls"),row.names=FALSE,col.names=TRUE,sep="\t",quote=FALSE)

## Identification of marker & differentially regulated genes in each cluster compared to the full ensemble
marttable <- read_tsv(params$conversiontable)
for (cluster in 1:length(sc@medoids)){
  ...
  write.table(x,file.path(params$outputdirectory,paste(paste("cell_clust_diff_genes",sub("\\.","\\_",cluster),sep="_"),".xls",sep="")),row.names=FALSE,col.names=TRUE,sep="\t",quote=FALSE)}

# Stop if StemID is not needed
if (!params$RunStemID) {
  save.image(file.path(params$outputdirectory,"R Workspace.RData"))
  knitr::knit_exit()}
