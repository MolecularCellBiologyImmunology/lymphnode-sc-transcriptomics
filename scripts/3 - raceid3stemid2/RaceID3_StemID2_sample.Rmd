...
for (cluster in 1:length(sc@medoids)){
  ...
  y <- biomaRt::getBM(attributes = c("ensembl_gene_id", "mgi_symbol", "chromosome_name", "description"), filters = "mgi_symbol", values = as.list(x['mgi_symbol']), mart = mart)
  z <- left_join(x, y, by = "mgi_symbol") # %>% select(-ensembl_gene_id)
