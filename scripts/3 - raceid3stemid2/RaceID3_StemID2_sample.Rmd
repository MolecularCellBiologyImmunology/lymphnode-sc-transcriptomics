...
types <- sub("LNS.*_.*\\.*","Pilot",sub("LNS_W", "Week ", sub("_P.*_.*\\.*","", colnames(sc@ndata))))
...
for (cluster in 1:length(sc@medoids)){
  ...
  x <- data.frame('ensembl_gene_id'=rownames(cdiff),cdiff)
  y <- biomaRt::getBM(attributes = c("ensembl_gene_id", "mgi_symbol", "chromosome_name", "description"), filters = "mgi_symbol", values = as.list(x['ensembl_gene_id']), mart = mart)
  z <- left_join(x, y, by = "ensembl_gene_id") # %>% select(-ensembl_gene_id)
