...
sc <- filterdata(sc, mintotal=1500, minexpr=5, minnumber=1, maxexpr=500, downsample=FALSE, sfn=FALSE, hkn=FALSE, dsn=1, rseed=17000, CGenes=TRUE, FGenes=NULL, ccor=.4)
...
write.table(x[order(x$cluster,decreasing=FALSE),],"data/4 - raceidstemid/German/cell_clust.xls",row.names=FALSE,col.names=TRUE,sep="\t",quote=FALSE)
  
# differentially expressed genes in cluster
for ( n in names(cdiff) ) write.table(data.frame(GENEID=rownames(cdiff[[n]]),cdiff[[n]]),paste(paste("data/4 - raceidstemid/German/cell_clust_diff_genes",sub("\\.","\\_",n),sep="_"),".xls",sep=""),row.names=FALSE,col.names=TRUE,sep="\t",quote=FALSE)
...
ltr <- projback(ltr,pdishuf=10000,nmode=FALSE,fast=FALSE,rseed=17000)            ### NOTE: SHOULD BE CHANGEABLE IN SNAKEMAKE
