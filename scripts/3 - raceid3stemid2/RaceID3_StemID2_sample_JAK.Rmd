...
setwd("D:/Userdata/jj.koning/MIKE/Seperate Scripts/scripts/3 - raceid3stemid2")
...
## input data
x <- read.csv("D:/Userdata/jj.koning/MIKE/Seperate Scripts/data/3 - combinedcounts/LNS_W2_unfiltered_combined.csv",sep=",",header=TRUE)
#x <- read.csv(file.choose(),sep=",",header=TRUE)
#x <- read.csv("C:/Users/Mike/Documents/WORK/Bioinformatics Project Internship/Scripts/seperate-scripts/lymphnode-sc-transcriptomics/data/3 - combinedcounts/LNS_W2_unfiltered_combined.csv",sep=",",header=TRUE)
rownames(x) <- x$GENEID
...
sc <- filterdata(sc, mintotal=1500, minexpr=5, minnumber=1, maxexpr=500, downsample=FALSE, sfn=FALSE, hkn=FALSE, dsn=1, rseed=17000, CGenes=c("Pcna","Mki67","Malat1","Hspa1a","Jun","Fos"), FGenes=NULL, ccor=.4)
...
plotsymbolstsne(sc,types=sub("(^.*_.*\\.X\\d+)$","Pilot (W2)",(sub("(_P.*_.*\\.*\\d+)$","", names(sc@ndata)))))
...
write.table(x[order(x$cluster,decreasing=FALSE),],"D:/Userdata/jj.koning/MIKE/Seperate Scripts/data",row.names=FALSE,col.names=TRUE,sep="\t",quote=FALSE)
  
# differentially expressed genes in cluster
for ( n in names(cdiff) ) write.table(data.frame(GENEID=rownames(cdiff[[n]]),cdiff[[n]]),paste(paste("D:/Userdata/jj.koning/MIKE/Seperate Scripts/data/4 - raceidstemid/Update_Unfiltered",sub("\\.","\\_",n),sep="_"),".xls",sep=""),row.names=FALSE,col.names=TRUE,sep="\t",quote=FALSE)
...
ltr <- projback(ltr,pdishuf=1000,nmode=FALSE,fast=FALSE,rseed=17000)            ### NOTE: SHOULD BE CHANGEABLE IN SNAKEMAKE
